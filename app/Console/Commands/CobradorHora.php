...
class CobradorHora extends Command
{
     ...
     * @var string
     */
    protected $signature = 'app:cobrador-hora';
     ...
     * @var string
     */
    protected $description = 'Genera cobros por hora para servicios con frecuencia horaria vigentes';
    ...
    {
        $this->info('🔄 Iniciando generación de cobros por hora...');
        
        $fechaHoy = Carbon::now();
        $cobrosGenerados = 0;
        
        // Obtener servicios por hora con sus clientes vigentes
        Servicio::where('tiempo', 'hora')
            ...
            ->each(function($servicio) use ($fechaHoy, &$cobrosGenerados) {
                ...
                $servicio->Clientes->each(function($cliente) use ($servicio, $fechaHoy, &$cobrosGenerados) {
                    ...
                    // Verificar si ya existe un cobro para esta hora
                    $yaExiste = ServicioPagar::where('cliente_id', $cliente->id)
                        ->where('servicio_id', $servicio->id)
                        ->where('created_at', '>=', $fechaHoy->copy()->startOfHour())
                        ->where('created_at', '<', $fechaHoy->copy()->endOfHour())
                        ->exists();
                    ...
                    if (!$yaExiste) {
                        ...
                        $servicioPagar = ServicioPagar::create([
                            ...
                            'comentario' => "Cobro automático generado - Servicio por hora",
