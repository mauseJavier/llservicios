...
<html lang="es">
  <head>
    ...
    <title>Restablecer Contraseña - {{env('APP_NAME')}}</title>
  </head>
  <body>
    ...
    <main class="">
      ...
      <div style="width: 100%; max-width: 400px;">
        ...
        <form method="POST" action="{{route('password.update')}}">
          ...
          <label for="email">
            Correo Electrónico
            <input type="email" id="email" name="email" placeholder="correo@ejemplo.com" value="{{old('email', $email)}}" required autofocus>
          </label>
        ...
        @if (session('status'))
        <div style="text-align: center; margin-top: 1rem;">
          <article style="background-color: #d4edda; border-color: #c3e6cb; color: #155724;">
            {{session('status')}}
          </article>
        </div>
        @endif
